...
function startGame() {
    ...
    // Show loading screen briefly before starting
    setTimeout(() => {
        hideLoadingScreen();
    }, 2000);
}
...
// Modify the retry button to show lobby instead of directly restarting
document.querySelector("#retry").addEventListener("click", () => {
    ...
    // Reset UI
    livesDOM.innerHTML = '❤️'.repeat(lives);
...
// Modify the animation function to only run when game is started
function animate(timestamp) {
    ...
    if(lanes[currentLane].type === 'car' || lanes[currentLane].type === 'truck') {
        ...
        lanes[currentLane].vechicles.forEach(vechicle => {
            ...
            if(chickenMaxX > carMinX && chickenMinX < carMaxX && !isInvincible) {
                lives--;
                livesDOM.innerHTML = '❤️'.repeat(lives);
